...
function SearchZoomRequest(pageSizeZoom, pageNumberZoom, sortBy, sortType, name, meetingId){
...
function searchZoom() {
    ...
    pageNumberRoom = 1;
    zoomName = document.getElementById("searchname").value; 
    getListZoom();
}
...
async function getListZoom() {
    let request = new SearchZoomRequest(pageSizeZoom, pageNumberZoom - 1, sortBy, sortType, zoomName, meetingId);
    $.ajax({
        url: apiZoom + "/search",
        type: "POST",
